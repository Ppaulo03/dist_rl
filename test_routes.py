...
def compare_routes(num_points=20, pos_range_x=(-50, 50), pos_range_y=(-50, 50)):
    ...
    # Nearest Neighbor
    start = time.time()
    nn_route = nearest_neighbor(origin, points, point_indices)
    nn_dist = total_distance(nn_route, points, origin)
    ...
    opt_dist = total_distance(opt_route, points, origin)
    ...
    ga_dist = total_distance(ga_route, points, origin)
    ga_time = time.time() - start
    print(f"Distância Genetic Algorithm: {ga_dist:.2f} km | Tempo: {ga_time:.2f} s")

    # NN + Genetic Algorithm
    nn_ga_route = genetic_algorithm(points, origin, generations=400, pop_size=200, mutation_rate=0.05, seeding=nn_route, seeding_size=20)
    nn_ga_dist = total_distance(nn_ga_route, points, origin)
    ...
    ga_opt_dist = total_distance(ga_opt_route, points, origin)
    ...
    nn_ga_opt_dist = total_distance(nn_ga_opt_route, points, origin)
...
def time_complexity(min_points=10, max_points=200, step=10, pos_range_x=(-50, 50), pos_range_y=(-50, 50)):
    ...
    for n in sizes:
        ...
        start = time.time()
        nn_route = nearest_neighbor(origin, points, point_indices)
        nn_ga_route =  genetic_algorithm(points, origin, generations=generations, pop_size=pop_size, mutation_rate=0.05, seeding=nn_route, seeding_size=seeding_size)
        ...
        results.append({
            "Pontos": n,
            "NN_Dist": round(nn_dist, 2), "2Opt_Dist": round(opt_dist, 2), "GA_Dist": round(ga_dist, 2),
            "NN_GA_Dist": round(nn_ga_dist, 2), "GA_2Opt_Dist": round(ga_opt_dist, 2), "NN_GA_2Opt_Dist": round(nn_ga_opt_dist, 2),
            "NN_Time": round(nn_time, 4), "2Opt_Time": round(opt_time, 4), "GA_Time": round(ga_time, 4),
            "NN_GA_Time": round(nn_ga_time, 4), "GA_2Opt_Time": round(ga_opt_time, 4), "NN_GA_2Opt_Time": round(nn_ga_opt_time, 4),
        })
